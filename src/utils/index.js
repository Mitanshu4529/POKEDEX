//so basically utils folder is used for storing and managing data which is to be used in my app which comes from some other source or like api or some shit so that i dont have to visit the api server again and again or else may cause a reason to get banned on their server

//NOTE: make sure that u r only using caching when the data that has to be used changes very infrequently or else the thing is that once u have stored that data in your cache now the previous data will be used the new data which is important or more correct is not used
...
export function getFullPokedexNumber(index) {
    return `${index + 1 > 99 ? index + 1 : index + 1 > 9 ? `${index + 1}` : `${index + 1}`}`
}